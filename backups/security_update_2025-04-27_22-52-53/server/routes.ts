...
import { z } from "zod";
import { insertBetSchema, insertDrawSchema, insertUserSchema, insertGameModeSchema, insertPaymentGatewaySchema, insertPaymentTransactionSchema, bets, paymentTransactions, BetWithDetails, Draw } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/bets", requireAuth, async (req, res) => {
    try {
      ...
      // Agora que os dados estão completamente validados, podemos enriquecer com detalhes
      const betsWithDetails: BetWithDetails[] = [];
      
      for (const bet of userBets) {
        try {
          // Buscar sorteio associado
          const draw = await storage.getDraw(bet.drawId);
          if (!draw) continue;
          
          // Buscar animal se existir
          const animal = bet.animalId ? await storage.getAnimal(bet.animalId) : undefined;
          const animal2 = bet.animalId2 ? await storage.getAnimal(bet.animalId2) : undefined;
          const animal3 = bet.animalId3 ? await storage.getAnimal(bet.animalId3) : undefined;
          const animal4 = bet.animalId4 ? await storage.getAnimal(bet.animalId4) : undefined;
          const animal5 = bet.animalId5 ? await storage.getAnimal(bet.animalId5) : undefined;
          
          // Buscar modalidade do jogo
          const gameMode = bet.gameModeId ? await storage.getGameMode(bet.gameModeId) : undefined;
          ...
          console.error(`Erro ao buscar detalhes da aposta ${bet.id}:`, detailError);
          // Incluir a aposta mesmo sem alguns detalhes
          betsWithDetails.push({
            ...bet,
            draw: { id: bet.drawId } as Draw // Garantimos pelo menos o ID do sorteio
          });
        }
      }
      
      console.log(`RESPOSTA: Enviando ${betsWithDetails.length} apostas para usuário ${username} (${userId})`);
  ...
  app.get("/api/payment-transactions", requireAuth, async (req, res) => {
    try {
      ...
      console.log(`SEGURANÇA: Operação concluída com sucesso. Retornando ${userTransactions.length} transações para usuário ${username} (${userId})`);
      ...
      // A resposta já está sanitizada pela função getUserTransactions
      console.log(`RESPOSTA: Enviando ${userTransactions.length} transações para usuário ${username} (${userId})`);
