...
export function BetForm({ selectedAnimal, upcomingDraws, onClearSelection }: BetFormProps) {
  ...
  const { user } = useAuth();
  // Inicializar explicitamente com string vazia e garantir que ela permaneça vazia
  const [betAmount, setBetAmount] = useState<string>("");
  
  // UseEffect para garantir que o valor inicial seja vazio
  useEffect(() => {
    setBetAmount("");
  }, []);
  ...
  useEffect(() => {
    if (gameModes && selectedGameMode && systemSettings) {
      ...
      if (gameMode) {
        ...
        // Usando multiplicação direta da odds com o valor da aposta
        const winAmount = betAmountNum * gameMode.odds;
  ...
  const handleBet = () => {
    ...
    if (systemSettings) {
      ...
      if (selectedMode && systemSettings.maxPayout) {
        // Calcular potencial de vitória com valores reais
        const potentialWin = betAmountNum * selectedMode.odds;
    ...
    if (premioType === "1-5") {
      ...
      console.log(`Aposta em todos os prêmios (1-5): Ganho potencial ajustado para ${finalPotentialWinAmount}`);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <div className="space-y-2">
            <MoneyInput
              id="bet-amount"
              min={systemSettings?.minBetAmount ? systemSettings.minBetAmount.toFixed(2) : "0,10"}
              max={systemSettings?.maxBetAmount ? systemSettings.maxBetAmount.toFixed(2) : undefined}
              value={betAmount}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-green-600">
                {premioType === "1-5" 
                  ? formatCurrency(potentialWinAmount / 5)
                  : formatCurrency(potentialWinAmount)
                }
                {premioType === "1-5" && (
              ...
              {systemSettings?.maxPayout && (
                <p className="text-xs text-gray-400">Máximo: {formatCurrency(systemSettings.maxPayout)}</p>
              )}
            </div>
          </div>
        </CardContent>
