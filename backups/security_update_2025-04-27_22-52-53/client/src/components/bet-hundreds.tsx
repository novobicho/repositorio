...
export function BetHundreds({ 
  ...
  // Encontrar a modalidade específica para Centena
  const hundredGameMode = gameModes?.find(mode => 
  ...
  const formSchema = z.object({
    ...
    gameModeId: z.literal(2),
    type: z.literal("hundred"),
    betNumber: z.string()
      .min(3, "A centena deve ter exatamente 3 dígitos")
      .max(3, "A centena deve ter exatamente 3 dígitos")
      .regex(/^[0-9]{3}$/, "A centena deve conter exatamente 3 dígitos numéricos"),
    premioType: z.enum(["1", "2", "3", "4", "5", "1-5"], {
      required_error: "Selecione um prêmio",
    })
  });

  // Initialize form - SEMPRE usa o ID 2 da Centena
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      drawId: selectedDraw || undefined,
      amount: undefined, // IMPORTANTE: Deve ser undefined para começar realmente vazio
      gameModeId: 2, // ID fixo para Centena
      type: "hundred",
      betNumber: "",
      premioType: "1"
    },
  ...
  useEffect(() => {
    ...
    console.log("bet-hundreds: Forçando valor inicial para vazio");
  ...
  const isBetNumberValid = formValues.betNumber && formValues.betNumber.length === 3;
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    if (!data.betNumber || data.betNumber.length !== 3) {
      toast({
        title: "Número incompleto",
        description: "Apostas de centena exigem exatamente 3 dígitos. Por favor, complete o número.",
        variant: "destructive",
      });
      return;
    }
    ...
    const betData: BetFormData = {
      drawId: data.drawId,
      amount: data.amount,
      gameModeId: 2, // Força o ID correto para Centena
      type: "hundred", // Força o tipo correto consistente com o ID
      premioType: data.premioType,
      potentialWinAmount,
      betNumbers: [data.betNumber], // Array com o número exato (sem preenchimento com zeros)
    };
    
    console.log("Enviando aposta de centena:", betData);
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-center mb-6">
            ...
            <h3 className="text-xl font-semibold ml-3">Apostar na Centena</h3>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <div className="mt-1 text-xs">
                          <span className={`${field.value && field.value.length === 3 ? 'text-green-500' : 'text-gray-400'}`}>
                            {field.value && field.value.length === 3 ? '✓' : '○'} Deve ter exatamente 3 dígitos
                          </span>
                        </div>
                              ...
                              onClick={() => {
                                ...
                                if (currentValue.length < 3) {
                                  ...
                                  toast({
                                    ...
                                    description: "Apostas de centena devem ter exatamente 3 dígitos.",
                                    variant: "destructive",
                                  });
                                }
                              }}
              ...
              {/* A modalidade é determinada pela aba - neste caso "Centena" */}
              {selectedGameMode && (
                <div className="bg-primary/5 p-3 rounded-lg">
                  <p className="text-sm text-gray-700">Modalidade:</p>
                  <p className="font-semibold text-primary flex items-center">
                    <span className="mr-2">Centena</span>
                    ...
                    <FormControl>
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="1" id="premio-1-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-1-hundreds"
                        ...
                        {/* Opção para 2° prêmio */}
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="2" id="premio-2-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-2-hundreds"
                            className="flex flex-col items-center justify-center w-full h-14 rounded-md border-2 border-muted 
                                     bg-popover p-1 hover:bg-accent hover:text-accent-foreground 
                                     peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10
                                     cursor-pointer text-center"
                        ...
                        {/* Opção para 3° prêmio */}
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="3" id="premio-3-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-3-hundreds"
                            className="flex flex-col items-center justify-center w-full h-14 rounded-md border-2 border-muted 
                                     bg-popover p-1 hover:bg-accent hover:text-accent-foreground 
                                     peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10
                                     cursor-pointer text-center"
                        ...
                        {/* Opção para 4° prêmio */}
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="4" id="premio-4-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-4-hundreds"
                            className="flex flex-col items-center justify-center w-full h-14 rounded-md border-2 border-muted 
                                     bg-popover p-1 hover:bg-accent hover:text-accent-foreground 
                                     peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10
                                     cursor-pointer text-center"
                        ...
                        {/* Opção para 5° prêmio */}
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="5" id="premio-5-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-5-hundreds"
                            className="flex flex-col items-center justify-center w-full h-14 rounded-md border-2 border-muted 
                                     bg-popover p-1 hover:bg-accent hover:text-accent-foreground 
                                     peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10
                                     cursor-pointer text-center"
                        ...
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="1-5" id="premio-1-5-hundreds" className="peer sr-only" />
                          <Label
                            htmlFor="premio-1-5-hundreds"
                            className="flex flex-col items-center justify-center w-full h-14 rounded-md border-2 border-muted 
                                     bg-popover p-1 hover:bg-accent hover:text-accent-foreground 
                                     peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10
                                     cursor-pointer text-center"
              ...
              <div className="pt-2">
                ...
                >
                  {betMutation.isPending ? (
                    ...
                    <>Digite exatamente 3 dígitos</>
                  ) : (
                    <>
                      <Check className="mr-2 h-4 w-4" /> Confirmar Aposta na Centena {formValues.betNumber}
                    </>
                  )}
                </Button>
              </div>
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  Aposte nos últimos 3 dígitos do número sorteado. Por exemplo, se o número sorteado for 1234, a centena ganhadora é <strong>234</strong>.
                </p>
              </div>
            </form>
          </Form>
