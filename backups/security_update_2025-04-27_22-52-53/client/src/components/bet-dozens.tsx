...
export function BetDozens({ 
  ...
  // Encontrar a modalidade específica para Dezena
  const dozenGameMode = gameModes?.find(mode => 
  ...
  const formSchema = z.object({
    ...
    gameModeId: z.literal(4),
    type: z.literal("dozen"),
    betNumber: z.string()
      .min(2, "A dezena deve ter exatamente 2 dígitos")
      .max(2, "A dezena deve ter exatamente 2 dígitos")
      .regex(/^[0-9]{2}$/, "A dezena deve conter exatamente 2 dígitos numéricos"),
    premioType: z.enum(["1", "2", "3", "4", "5", "1-5"], {
      required_error: "Selecione um prêmio",
    })
  });

  // Initialize form - SEMPRE usa o ID 4 da Dezena
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      drawId: selectedDraw || undefined,
      amount: undefined, // IMPORTANTE: Deve ser undefined para começar realmente vazio
      gameModeId: 4, // ID fixo para Dezena
      type: "dozen",
      betNumber: "",
      premioType: "1"
    },
  ...
  useEffect(() => {
    ...
    console.log("bet-dozens: Forçando valor inicial para vazio");
  ...
  const isBetNumberValid = formValues.betNumber && formValues.betNumber.length === 2;
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    if (!data.betNumber || data.betNumber.length !== 2) {
      toast({
        title: "Número incompleto",
        description: "Apostas de dezena exigem exatamente 2 dígitos. Por favor, complete o número.",
        variant: "destructive",
      });
      return;
    }
    ...
    const betData: BetFormData = {
      drawId: data.drawId,
      amount: data.amount,
      gameModeId: 4, // Força o ID correto para Dezena
      type: "dozen", // Força o tipo correto consistente com o ID
      premioType: data.premioType,
      potentialWinAmount,
      betNumbers: [data.betNumber], // Array com o número exato (sem preenchimento com zeros)
    };
    
    console.log("Enviando aposta de dezena:", betData);
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-center mb-6">
            ...
            <h3 className="text-xl font-semibold ml-3">Apostar na Dezena</h3>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <div className="mt-1 text-xs">
                          <span className={`${field.value && field.value.length === 2 ? 'text-green-500' : 'text-gray-400'}`}>
                            {field.value && field.value.length === 2 ? '✓' : '○'} Deve ter exatamente 2 dígitos
                          </span>
                        </div>
                              ...
                              onClick={() => {
                                ...
                                if (currentValue.length < 2) {
                                  ...
                                  toast({
                                    ...
                                    description: "Apostas de dezena devem ter exatamente 2 dígitos.",
                                    variant: "destructive",
                                  });
                                }
                              }}
              ...
              {/* A modalidade é determinada pela aba - neste caso "Dezena" */}
              {selectedGameMode && (
                <div className="bg-primary/5 p-3 rounded-lg">
                  <p className="text-sm text-gray-700">Modalidade:</p>
                  <p className="font-semibold text-primary flex items-center">
                    <span className="mr-2">Dezena</span>
              ...
              <div className="pt-2">
                ...
                >
                  {betMutation.isPending ? (
                    ...
                    <>
                      <Check className="mr-2 h-4 w-4" /> Confirmar Aposta na Dezena {formValues.betNumber}
                    </>
                  )}
                </Button>
              </div>
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  Aposte nos últimos 2 dígitos do número sorteado. Por exemplo, se o número sorteado for 1234, o dezena ganhadora é <strong>34</strong>.
                </p>
              </div>
            </form>
          </Form>
